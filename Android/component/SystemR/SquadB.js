...
export default function SquadB( {route,navigation})
{
      ...
      else
      {
        ...
        else
        {
          ...
          const team_data = 
            ...
            navigation.navigate('SystemPredictionResult',{ user: team_data  });
  ...
  const AddPlayers = (Name, player_type) =>
    ...
    const renderedBatsmen =  Batsmen_List.filter(name => name["Country"] === Team_2_name).map((batsman,index) =>  {
      return (
        <TouchableOpacity 
          style={{
            marginTop:10,
    ...
    const renderedBowlers =  Bowlers_List.filter(name => name["Country"] === Team_2_name).map((bowler,index) => {
      return( 
        <TouchableOpacity
          style={{
            marginTop:10,
    ...
    return (
      <View style={{flex:1,backgroundColor:'#fff',alignItems:'center'}}>
        <Text style={{marginTop:20, fontSize:18,fontWeight:'bold'}}>Team - B : {Team_2_name}</Text>
        ...
        <View style={{flex:2, flexDirection:"row"}}>
          ...
          <View style={{flex: 1, alignItems:'center',height:'95%'}}>
            <Text style={{marginTop:25, marginBottom:15}}>Bowler</Text>
...

    {
     ...
     <TouchableOpacity   onPress= {() => DeletePlayer(item.Name)} key={index} ><Text style = {{"color" : "blue"}} >{index + 1} : {item.Name}</Text></TouchableOpacity>
  ) : (<TouchableOpacity   onPress= {() => DeletePlayer(item.Name)} key={index} ><Text style = {{"color" : "green"}}>{index + 1} : {item.Name}</Text></TouchableOpacity>))}</View>) : (null)
    }       

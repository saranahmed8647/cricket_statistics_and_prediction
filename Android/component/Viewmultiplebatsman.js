...
export default function Viewmultiplebatsman({navigation})
{
  ...
  useEffect(() =>
  {
    ...
    if(token)
    {
      axios.get(`http://192.168.0.107:5000/account/verify?token=${token}`)
        ...
        axios.get('http://192.168.0.107:5000/player/batsmen')
    ...
    return (
      ...
      <Animatable.View style={{}} animation='fadeInDownBig'>
        ...
        <FlatList 
          ...
          data={TeamsData}
          renderItem={({item}) => (
            <TouchableOpacity onPress={() => navigation.navigate('BatsmanData',{ user: item.name  })}>
              <Card style={{marginBottom:10}}>
                <View style={{ alignItems: 'center',flex: 2,flexDirection:'row'}}>
                  <Image style={{marginLeft:15,height:30,width:30}} source={require('../assets/bat.png')}/>

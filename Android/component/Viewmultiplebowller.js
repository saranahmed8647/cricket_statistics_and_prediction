...
export default function Viewmultiplebowller({navigation})
{
  ...
  useEffect(() =>
  {
    ...
    if(token)
    {
      axios.get(`http://192.168.0.107:5000/account/verify?token=${token}`)
        ...
        axios.get('http://192.168.0.107:5000/player/bowlers')
    ...
    return (
      ...
      <Animatable.View style={{}} animation='fadeInDownBig'>
        ...
        <FlatList 
          ...
          data={TeamsData}
          renderItem={({item}) => (
            <TouchableOpacity onPress={() => navigation.navigate('BowllerData',{ user: item.name  })}>
              <Card style={{marginBottom:10}}>
                <View style={{ alignItems: 'center',flex: 2,flexDirection:'row'}}>
                  <Image style={{marginLeft:15,height:50,width:50}} source={require('../assets/ball.png')}/>
                  <Text style={{color:'#00bfff',fontWeight:'bold',fontSize:15,textAlign:'center',padding:10,margin:5}}>{item.name}</Text>
                </View>  
              </Card>

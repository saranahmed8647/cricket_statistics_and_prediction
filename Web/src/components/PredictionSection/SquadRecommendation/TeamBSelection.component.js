...
const TeamBSelection = (props) => 
{
  ...
  useEffect(() =>
    ...
    const SubmittedFunc = () =>
    {
      ...
      else
      {
        ...
        else
        {
          ...
          history.push({
            pathname : '/SquadRecommendationResult',
            team_data :{
                Team1: Team_1_name,
                Team2: Team_2_name,
                Venue: Venue,
                Toss : Toss,
                Squad2 : selectedSquad   
            }
            } 
          );
    ...
    const AddPlayers = (Name, player_type) =>
    {
      ...
      {
        alert("Cant add more than 11 Players");
      }
...
//         <div class="card border-primary mb-3" style={{width:'40rem',marginTop:'5%',display:'flex',flex:1,alignItems:'center'}}>
...
//                     <p style={{marginTop : '5%', marginBottom:'5%', color:'#009387',fontSize:20}}>Batsmen</p>
//                     <div style={{marginLeft:'25%',height: '20rem', overflowY: 'scroll'}}>{rendered_Batsmen_Buttons}</div>
//                 </div>

//                 <div style={{display:'flex',flex:1,flexDirection:'column',alignItems:'center'}}>
//                     <p style={{marginTop : '5%', marginBottom:'5%', color:'#009387',fontSize:20}}>Bowlers</p>
//                     <div style={{marginLeft:'25%',height: '20rem', overflowY: 'scroll'}}>{rendered_Bowlers_Buttons}</div>
//                 </div>
//             </div>

//             {/* Call TeamB.js */}
//             <button type="button" onClick={SubmittedFunc} class="btn btn-primary" style={{width:'20rem',marginBottom:'5%',marginTop:'5%'}}>
//                 Select Team - B
//             </button>
//             <p><strong>Selected Player : Batsman  = Blue ---- Bowler  = Green</strong></p>
    
//     {/* {
        
//         (selectedSquad.length > 0) ? (<p>{selectedSquad.map((item, index) => (
//      <p>{item.Name}</p>
//   ))}</p>) : (null)
//     }        */}
...
// <p style = {{"color" : "blue"}} onClick= {() => DeletePlayer(item.Name)}>{index + 1} : {item.Name}</p>
// ) : (<p style = {{"color" : "green"}} onClick= {() => DeletePlayer(item.Name)} >{index + 1} : {item.Name}</p>))}</p>) : (null)
// }       
...
<div class="card" style={{marginLeft:'30%',width:'60rem',marginTop:'5%',marginBottom:'5%',display:'flex',flex:1,alignItems:'center',height:'100%'}}>
      ...
      <p style={{marginTop : '5%', marginBottom:'5%', color:'#e60073',fontSize:25,fontWeight:'bold'}}>Team - B Selection : {Team_2_name}</p>
      <p style={{color:'#b30059',fontSize:15,fontWeight:'bold'}}>Select minimum 5 Bowlers to continue...</p>
      <div style={{display:'flex',flex:2, flexDirection:"row",alignItems:'center'}}>
          <div style={{display:'flex',flex:1, flexDirection:'column',alignItems:'center'}}>
      ...
      (selectedSquad.length > 0) ? (<p>{selectedSquad.map((item, index) => (item.Type === "Batsman") ?(
      <p style = {{"color" : "blue",fontSize:13}} onClick= {() => DeletePlayer(item.Name)}>{index + 1} : {item.Name}</p>
      ) : (<p style = {{"color" : "green",fontSize:13}} onClick= {() => DeletePlayer(item.Name)} >{index + 1} : {item.Name}</p>))}</p>) : (null)
      }       

...
{  
  ...
  //    <p style = {{"color" : "blue"}} onClick= {() => DeletePlayer(item.Name)}>{item.Name}</p>
  // ) : (<p style = {{"color" : "green"}} onClick= {() => DeletePlayer(item.Name)} >{item.Name}</p>))}</p>) : (null)
  //   }       

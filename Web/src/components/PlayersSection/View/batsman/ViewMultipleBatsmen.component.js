        ...
        <table class="table table-striped">
          ...
          <tbody style={{textAlign:"center"}}>
            {items.map((item,index) => (
              <tr key={index}>
                <td >{index + 1}</td>
                <td style ={{"cursor" : "pointer"}} onClick={() =>{history.push(`/batsmenDetails/${item.name}`)}}><strong>{item.name}</strong></td>
...
else
{
  ...
  <div className="ui large text loader">Getting All Batsmen</div>
  </div>
  <p></p>
  <p></p>   
  <p></p>
...
export default ViewMultiplebatsmen;
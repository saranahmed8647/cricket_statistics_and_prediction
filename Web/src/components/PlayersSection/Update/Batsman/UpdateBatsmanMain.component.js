...
// Getting allrounders
    axios.get('http://localhost:5000/player/batsmen')
...
// If user is properly logged in
    ...
    <div style={{height:"100%"}} >
        ...
        (                        
            <div>
                <button style={{fontSize:15,color:'#00b36b',outline:'none',backgroundColor:'Transparent'}} onClick={() =>{history.push(`/UpdateOneBatsman/${team.name}`)}}><strong>{team.name}</strong></button>
                <hr />
            </div>
        ))}
        </div>
...
export default UpdateBatsmansMain;
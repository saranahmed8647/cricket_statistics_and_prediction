...
const AddMultipleBowlers = () =>
{
    ...
    const TeamHandler = (e) => 
    {
        ...
        else if((name.trim().endsWith(".csv")) || (name.trim().endsWith(".CSV")))
        {
          ...
          alert("Started Adding Bowlers to the DB ");
            const team_data = 
            {
                name: name.trim()
            }
            axios.post('http://localhost:5000/player/bowlers/addFile', team_data, {timeout : 650000})   
                .then(json =>
                    {   
                        if(json.status === 200)
                        {
                            ...
                            alert("Bowlers added successfully !");
                            history.push("/mainmenu");
                        }
                        else
                        {
                          alert("Error Adding Bowlers to DB, going to Main Menu");
                          history.push("/mainmenu");
                        }
                    })
...
<div class="ui middle aligned center aligned grid">
  <div class="column">
    <h2 class="ui image header">
      <div class="content">
      <span>Enter the File Name containing the Bowler's Data, <strong>without .csv</strong> </span>
      </div>
    </h2>
        ...
        <div className="field">
          <div className="ui left icon input">
            ...
            <input type="text" name="name" value={name} style = {{width : "100%"}}
                                                   aria-label="name"
                                                   title="Enter the File name with Bowler's data"
...
export default AddMultipleBowlers;
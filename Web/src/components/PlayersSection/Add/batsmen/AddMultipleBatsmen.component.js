    ...
    const TeamHandler = (e) => 
    {
        ...
        else if((name.trim().endsWith(".csv")) || (name.trim().endsWith(".CSV")))
        {
          setTeamDataErrors([...teamDataErrors,"Please write File name without .csv in it"]);
            ...
            alert("Started Adding Batsmen to the DB ");
            const team_data = 
            {
                name: name.trim()
            }
            axios.post('http://localhost:5000/player/batsmen/addFile', team_data, {timeout : 650000})   
                .then(json =>
                    {   
                        if(json.status === 200)
                        {
                            ...
                            alert("Batsmen added successfully !");
                            history.push("/mainmenu");
                        }
                        else
                        {
                          alert("Error adding Batsmen ! , going to Main Menu");
                            history.push("/mainmenu");
                        }
                    })
...
<div class="ui middle aligned center aligned grid">
  <div class="column">
    <h2 class="ui image header">
      <div class="content">
      <span>Enter the File Name containing the Batsmen's Data, </span><strong>without .csv</strong> 
      </div>
    </h2>
    <form action="post" onSubmit={TeamHandler} class="ui large form">
      <div className="ui stacked secondary  segment">
        ...
        <input type="text" name="name" value={name} style = {{width : "100%"}}
                                                   aria-label="name"
                                                   title="Enter the File name with Batsmen's data"
...
export default AddMultipleBatsmen;